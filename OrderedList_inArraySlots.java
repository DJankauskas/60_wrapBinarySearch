 ...
 * 
 * A list with the invariant the elements are in order by increasing value,
 * implemented using a ArrayList<Integer>
 */
...
public class OrderedList_inArraySlots implements OrderedList {
    ...
    private int indexOf_whileStyle(Integer findMe) {
        ...
        while (low <= hi) {
            ...
            cost += 1; // for compareTo
            int comparison = findMe.compareTo(list_iAS.get(pageToCheck));

            cost++; // for next comparison
            if (comparison == 0)
                return pageToCheck;
            else {
                cost += 1; // comparison will be performed
                if (comparison < 0)
        ...
        cost++; //while comparison wasn't counted if we've reached here
